...
const DEFAULT_MENU = [
  ...
  // Africains
  { id: "af-1", name: "Riz sauce arachide (poulet)", category: "Plats Africains", price: 3e3 },
  ...
  { id: "fr-3", name: "Salade nicoise", category: "Plats Francais", price: 3e3 },
  { id: "fr-4", name: "Croque-monsieur", category: "Plats Francais", price: 2e3 },
  // Americains
  { id: "us-1", name: "Burger maison", category: "Plats Americains", price: 4e3 },
  ...
  { id: "co-2", name: "Mojito", category: "Cocktails & Boissons", price: 2e3 },
  { id: "co-3", name: "Pina Colada", category: "Cocktails & Boissons", price: 2200 },
  { id: "co-4", name: "Gin Tonic", category: "Cocktails & Boissons", price: 2e3 },
  { id: "co-5", name: "Biere locale (Castel, Flag)", category: "Cocktails & Boissons", price: 1e3 },
  ...
  // Snacks
  { id: "sn-1", name: "Chips et dips", category: "Snacks", price: 1e3 },
  { id: "sn-2", name: "Calamars frits", category: "Snacks", price: 2500 },
  { id: "sn-3", name: "Spring rolls", category: "Snacks", price: 1800 }
];
function useMenu() {
  const [items, setItems] = useLocalStorage("togoliving_menu", DEFAULT_MENU);
  const addItem = (item) => setItems([...items, { ...item, id: crypto.randomUUID() }]);
  const updateItem = (id, patch) => setItems(items.map((i) => i.id === id ? { ...i, ...patch } : i));
  const removeItem = (id) => setItems(items.filter((i) => i.id !== id));
  const toggleSoldOut = (id) => setItems(items.map((i) => i.id === id ? { ...i, soldOut: !i.soldOut } : i));
  return { items, setItems, addItem, updateItem, removeItem, toggleSoldOut };
}
export {
  MENU_CATEGORIES as M,
  useMenu as u
};
