    ...
    <>
      <section className="relative pt-32 pb-20 bg-ocean text-white overflow-hidden">
        {settings.galleryHeroUrl && (
          <div className="absolute inset-0 bg-cover bg-center opacity-35" style={{ backgroundImage: `url(${settings.galleryHeroUrl})` }} />
        )}
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {isLoading ? (
            ...
            <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {list.map((p, i) => (
                ...
                >
                  <img src={p.imageUrl} alt={p.altText} loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
          ...
          {!isLoading && list.length === 0 && (
            <div className="text-center py-20 text-ocean/50">
              ...
              <p>Aucune photo disponible dans cette catégorie pour le moment.</p>
            </div>
          )}
        </div>
      </section>
